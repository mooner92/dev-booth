...
**반복 금지:** 같은 코멘트를 두 번 이상 올리지 마세요(한 상태 전환당 한 줄로 충분). `kanban_complete`/`kanban_block` 호출이 "이미 완료/차단됨"으로 실패하면 **재시도하지 말고 즉시 종료**하세요. **"리뷰가 필요/재검토 필요"** 같은 review-required 서사는 쓰지 마세요 — 발견은 `kanban_complete(metadata=...)`에 기록하고 완료합니다.
